$('img').each(function() {
    var tempSrc = this.src.substring(this.src.indexOf('images'));
    this.src = "https://livedemo00.template-help.com/wt_prod-22320/" + tempSrc;
})

$('.layout-4-item-left').src('https://cdn.autoportal.com/bp-v3/img/models/9c/6/honda-activa-5g-1522148835.png');
...
(function() {
    ...
    CloudflareApps.querySelector = function(selector) {
        ...
        if (!els || !els.length) {
            ...
            if (!els || !els.length) { noteError({ type: 'select:not-found:by-query', selector: selector }); return null; } else if (els.length > 1) { noteError({ type: 'select:too-many:by-query', selector: selector }); }
            return els[0];
        }
    ...
    CloudflareApps.createElement = function(options, prevEl) {
        ...
        try {
            ...
            switch (options.method) {
                case "prepend":
                    if (container.firstChild) { container.insertBefore(element, container.firstChild); break; }
                case "append":
                    container.appendChild(element);
                    break;
                case "after":
                    if (container.nextSibling) { container.parentNode.insertBefore(element, container.nextSibling); } else { container.parentNode.appendChild(element); }
                    break;
                case "before":
...
(function() {
    ...
    script.src = 'https://livedemo00.template-help.com/cdn-cgi/apps/head/3ts2ksMwXvKRuG480KNifJ2_JNM.js';
